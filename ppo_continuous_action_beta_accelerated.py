...
if __name__=="__main__":
    ...
    for update in range (1, num_updates + 1):
        ...
        # Nesterov outer-loop reference policy : we replace b_logprobs (ratio denominator) with log π_lookahead(a|s), where θ_lookahead = θ_k + α · (θ_k − θ_{k-1})
        ...
        if args.nesterov_alpha > 0.0:
            ...
            with torch.no_grad():
                for name, p in agent.named_parameters():
                    ...
                    p.copy_(params_current[name] + args.nesterov_alpha * delta)
            # Recompute log π_lookahead for every (obs, action) pair in the batch
            with torch.no_grad():
            ...
            # Restore current parameters before inner optimization
            with torch.no_grad():
                for name, p in agent.named_parameters():
                    p.copy_(params_current[name])
        ...
        for epoch in range(args.update_epochs):
            ...
            b_values_old = values_epoch.reshape(-1) # values_epoch instead of values, as they are computed but not used
