...
if __name__=="__main__":
    ...
    # we store a copy of log π_{k-1}(a_t | s_t)
    if args.nesterov_alpha > 0.0:
        logprobs_prev = torch.zeros((args.num_steps, args.num_envs), device=device)
    else:
        logprobs_prev = None
    ...
    for update in range (1, num_updates + 1):
        ...
        # instead of extrapolating in θ-space we accumulate momentum directly on log π(a|s) 
        # log π_lookahead(a|s) = log π_k(a|s) + α · (log π_k(a|s) − log π_{k-1}(a|s))
        # KL(π_old ∥ π_lookahead) = -E[α · (log π_k − log π_{k-1})]
        # No forward pass needed: log π_k(a|s) = logprobs (already computed in rollout)
        if args.nesterov_alpha > 0.0:
            logprob_old = logprobs.reshape(-1)            # log π_k(a|s),  shape (B,)
            logprob_prev_flat = logprobs_prev.reshape(-1) # log π_{k-1}(a|s), shape (B,)

            # Extrapolate: project forward along the log-prob momentum direction
            logprob_delta = logprob_old - logprob_prev_flat   # how much log-prob moved this step
            b_logprobs = logprob_old + args.nesterov_alpha * logprob_delta  # lookahead reference

            logprob_diff = b_logprobs - logprob_old           # = α · logprob_delta
            old_to_lookahead_ratio = torch.exp(logprob_old - b_logprobs)   # π_old / π_lookahead
            ...
            # True forward KL from π_old to π_lookahead, estimated on rollout samples:
            # KL(π_old ∥ π_lh) = E_{π_old}[log π_old - log π_lh] = -E[logprob_diff]
            kl_lookahead = -logprob_diff.mean().item()
        ...
        for epoch in range(args.update_epochs):
            ...
            b_values_old = values.reshape(-1) # values_epoch instead of values, as they are computed but not used
        ...
        # log-prob buffer update
        if args.nesterov_alpha > 0.0:
            logprobs_prev = logprobs.clone()   # store log π_k for next iteration's delta
