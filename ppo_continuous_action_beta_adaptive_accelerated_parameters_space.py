...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        logprob_01 = dist.log_prob(action_01).sum(dim=1)  # log prob in normalized space
        logprob_env = logprob_01 - self.log_act_scale.sum() # affine transform correction

        entropy_env = dist.entropy().sum(dim=1) + self.log_act_scale.sum() # differential entropy under affine transform
...
def load_params(agent, param_dict):
    with torch.no_grad():
        for name, p in agent.named_parameters():
            p.copy_(param_dict[name])

def clone_params(agent):
...
def parse_args():
    ...
    # Nesterov outer loop momentum
    parser.add_argument('--nesterov-alpha', type=float, default=0.0, help='0 = standard PPO, >0 = use pi_lookahead as ratio reference')
    parser.add_argument('--nesterov-adaptive', type=lambda x: bool(strtobool(x)), default=False, nargs='?', const=True,
                        help='only accept the Nesterov lookahead when it improves the clipped surrogate; falls back to theta_{k+1} otherwise')
...
if __name__=="__main__":
    ...
    if args.nesterov_alpha > 0.0:
        theta_k   = clone_params(agent)          # θ_{k-1} for the first delta computation
        theta_ref = clone_params(agent)          # θ_ref = θ_0 on first iter 
    else:
        theta_k   = None
        theta_ref = None
    ...
    for update in range (1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            actions[step] = action
            logprobs[step] = logprob  # = log π_{θ_ref}(a|s)  
        ...
        # how far is θ_ref from θ_k
        if args.nesterov_alpha > 0.0:
            with torch.no_grad():
                delta_sq = sum(
                    (theta_ref[n] - theta_k[n]).pow(2).sum()
                    for n in theta_ref
                )
                param_sq = sum(
                    theta_ref[n].pow(2).sum()
                    for n in theta_ref
                )
                delta_norm    = torch.sqrt(delta_sq).item()
                relative_norm = delta_norm / (torch.sqrt(param_sq).item() + 1e-8)
            writer.add_scalar("debug/nesterov_ref_delta_norm",    delta_norm,    global_step)
            writer.add_scalar("debug/nesterov_ref_relative_norm", relative_norm, global_step)
        ...
        b_values_rollout = values.reshape(-1)   # rollout values

        # inner loop with  π_θ / π_{θ_ref}
        if args.nesterov_alpha > 0.0:
            load_params(agent, theta_k)  
        ...
        for epoch in range(args.update_epochs):
            ...
            b_values_old = b_values_rollout   # fixed rollout anchor
        ...
        if args.nesterov_alpha > 0.0:
            theta_kp1 = clone_params(agent)   # θ_{k+1}

            # θ_ref_{k+1} = θ_{k+1} + α·(θ_{k+1} − θ_k)
            theta_ref_candidate = {
                name: theta_kp1[name] + args.nesterov_alpha * (theta_kp1[name] - theta_k[name])
                for name in theta_kp1
            }
 
            # we'll compare the clipped surrogate objective evaluated at θ_{k+1} vs at θ_ref_candidate, both using π_{θ_ref} (the current rollout policy) as the denominator
            # b_logprobs = log π_{θ_ref} (current rollout denominator)
            # we'll use the full batch (no minibatching) for a stable estimate.
            if args.nesterov_adaptive:
                with torch.no_grad():
                    # Surrogate at θ_{k+1}  (agent currently holds θ_{k+1})
                    _, lp_kp1, _, _ = agent.get_action_and_value(b_obs, b_actions)
                    ratio_kp1 = (lp_kp1 - b_logprobs).exp()
                    adv_flat  = b_advantages  # already computed and normalised per minibatch above;
                                              # use raw (un-normalised) for a consistent full-batch estimate
                    adv_raw   = advantages.reshape(-1)  # last recompute_gae output
                    surr_kp1  = torch.min(
                        ratio_kp1 * adv_raw,
                        torch.clamp(ratio_kp1, 1 - args.clip_coef, 1 + args.clip_coef) * adv_raw
                    ).mean().item()
 
                    # Surrogate at θ_ref_candidate
                    load_params(agent, theta_ref_candidate)
                    _, lp_cand, _, _ = agent.get_action_and_value(b_obs, b_actions)
                    ratio_cand = (lp_cand - b_logprobs).exp()
                    surr_cand  = torch.min(
                        ratio_cand * adv_raw,
                        torch.clamp(ratio_cand, 1 - args.clip_coef, 1 + args.clip_coef) * adv_raw
                    ).mean().item()
 
                    # Restore θ_{k+1} into agent before the accept/reject decision
                    load_params(agent, theta_kp1)
 
                nesterov_accepted = surr_cand > surr_kp1
                theta_ref_next = theta_ref_candidate if nesterov_accepted else theta_kp1
 
                writer.add_scalar("debug/nesterov_surr_kp1", surr_kp1, global_step)
                writer.add_scalar("debug/nesterov_surr_candidate", surr_cand, global_step)
                writer.add_scalar("debug/nesterov_surr_delta", surr_cand - surr_kp1, global_step)
                writer.add_scalar("debug/nesterov_accepted", float(nesterov_accepted), global_step)
            else:
                theta_ref_next = theta_ref_candidate
 
            # KL between π_{θ_ref_next} and π_{θ_{k+1}}
            load_params(agent, theta_ref_next)
            with torch.no_grad():
                _, lp_ref_next, _, _ = agent.get_action_and_value(b_obs, b_actions)
            load_params(agent, theta_kp1)
            with torch.no_grad():
                _, lp_kp1_check, _, _ = agent.get_action_and_value(b_obs, b_actions)
            kl_ref_vs_corrected = (lp_kp1_check - lp_ref_next).mean().item()
            writer.add_scalar("debug/nesterov_kl_ref_vs_corrected", kl_ref_vs_corrected, global_step)
 
            # Advance state
            theta_k   = theta_kp1
            theta_ref = theta_ref_next
 
            load_params(agent, theta_ref) # load θ_ref into agent so that we collect next rollout with it
