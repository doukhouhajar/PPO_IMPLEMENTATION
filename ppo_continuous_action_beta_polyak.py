...
# cosine similarity between current(grad g_t and momentum buffet) computed after loss.backward() and before optimizer.step()
def grad_momentum_cosine(optimizer):
    grad_vecs, mom_vecs = [], []
    for group in optimizer.param_groups:
        for p in group['params']:
            ...
            if 'exp_avg' not in state:
                continue  # buffer not yet initialised (first step)
            grad_vecs.append(p.grad.detach().flatten())
            mom_vecs.append(state['exp_avg'].detach().flatten())
    ...
    for update in range (1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                loss.backward()
                # for cosim    
                #cos_sim = grad_momentum_cosine(optimizer)
        ...
        #if cos_sims:
            #writer.add_scalar("debug/grad_momentum_cosine", np.mean(cos_sims), global_step) 
        # debug critic 
